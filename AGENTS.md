...
**玩家移动**：行走 2.0 m/s，冲刺 3.5 m/s（体力 4.2 秒，耗尽后需回满
3.5 秒才能再冲），蹲下 1.4 m/s。猎杀期间冲刺速度不变，没有肾上腺素加成。

**鬼的猎杀速度刻意压在玩家冲刺附近**：怨灵 3.3、幽影 3.4、骚灵 3.6。
也就是说骚灵比玩家快，你**跑不过它**——逃跑不是答案，切断视线才是。
`GhostAI` 丢失视线后只会去最后消失点附近搜索，绕过拐角、关上身后的门
就能甩掉它。改这些数值前先读 `Docs/01_GDD_残响.md` 的「速度基准」一节，
那张表是唯一权威。
